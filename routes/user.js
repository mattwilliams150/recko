    ...
    User.findOne(
      ...
      (err, user) => {
        if (!user) {
          req.flash(
            "error",
            "Password reset link has expired or is invalid. Re-enter your email to get a new link."
          );
          return res.redirect("/forgot");
        }
        var errors = req.flash("error");
  ...
  app.post("/reset/:token", (req, res) => {
    async.waterfall([
      function (callback) {
        User.findOne(
          {
            passwordResetToken: req.params.token.substring(1),
            passwordResetExpires: { $gt: Date.now() },
          },
          (err, user) => {
            if (!user) {
